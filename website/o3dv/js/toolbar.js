OV.ToolbarButton = class
{
    ...
    CreateDomElementMe (parentDiv)
    {
        this.buttonDiv = $('<div>').addClass ('ov_toolbar_button').attr('style', 'margin-left:5px;').prependTo(parentDiv);
        this.buttonImg = OV.CreateSvgIconMe (this.buttonDiv, this.image);
        if (this.onClick !== null) {
            this.buttonDiv.click (this.onClick);
        }
        this.buttonDiv.attr ('alt', this.imageTitle);
...
OV.Toolbar = class
{
    ...
    AddImageButtonMe (image, imageTitle, onClick)
    {
        let button = new OV.ToolbarButton (image, imageTitle, onClick);
        button.CreateDomElementMe (this.mainDiv);
        return button;
    }
