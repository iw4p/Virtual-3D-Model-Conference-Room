...
OV.CreateSvgIconMe = function (parent, iconName, extraClass)
{
    let div = $('<div>').addClass ('ov_svg_icon').attr('style', 'width: 20px').attr('style', 'display:flex;justify-content: center;align-items: center;').appendTo (parent);
    $('<img>').attr('src', 'assets/images/menu_black_24dp.svg').attr('style', 'height: 23px;width:23px;').text(iconName).appendTo (div);
    if (extraClass !== undefined && extraClass !== null) {
        div.addClass (extraClass);
    }
    return div;
