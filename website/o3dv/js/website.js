...
OV.Website = class {
    constructor(parameters) {
        ...
        this.viewer = new OV.Viewer(this.parameters.socket);
    ...
    Load() {
        ...
        //     return;
        // }
        let urls = ['assets/models/Tester_Case.glb'];
        //let urls = ['https://modelviewer.dev/shared-assets/models/shishkebab.glb'];
    ...
    LoadModel(name) {
        let url = 'assets/models/' + name + '.glb';
        //let url = 'https://modelviewer.dev/shared-assets/models/' + name + '.glb';
        let urls = [url];
        let importSettings = new OV.ImportSettings();
        importSettings.defaultColor = this.settings.defaultColor;
        let defaultColor = this.hashHandler.GetDefaultColorFromHash();
    ...
    Resize() {
        ...
        let contentWidth = windowWidth - sidebarWidth - safetyMargin;
        ...
        let myHeight = contentHeight - 20;
        $('.main_navigator').attr('style', 'display:none;overflow:auto;height:' + myHeight + 'px;');
    ...
    InitToolbar() {
        ...
        // AddButtonMe (this.toolbar, this.eventHandler, 'menu_black_24dp', 'Open Menu', null, () => {
    ...
    InitModelLoader() {
        function AddButtonMe(toolbar, eventHandler, imageName, imageTitle, extraClass, viewer, onClick) {
            let button = toolbar.AddImageButtonMe(imageName, imageTitle, () => {
                eventHandler.HandleEvent('toolbar_clicked', {item: imageName});
                onClick();

                let windowWidth = $(window).outerWidth();
                let windowHeight = $(window).outerHeight();
                let headerHeight = parseInt($('#header').outerHeight(true), 10);

                // let navigatorWidth = 0;
                // let sidebarWidth = 0;
                // let safetyMargin = 0;
                // if (!OV.IsSmallWidth ()) {
                //     navigatorWidth = parseInt ($('#main_navigator').outerWidth (true), 10);
                //     // if (this.sidebar.IsVisible ()) {
                //     //     sidebarWidth = parseInt ($('#main_sidebar').outerWidth (true), 10);
                //     // }
                //     safetyMargin = 1;
                // }

                let contentWidth;
                $('#main_sidebar').attr('style', 'display:none');

                let contentHeight = windowHeight - headerHeight;
                let myHeight = contentHeight - 20;

                if ($('#main_navigator').css('display') === 'block') {
                    $('.main_navigator').attr('style', 'display:none;overflow:auto;height:' + myHeight + 'px;');
                    contentWidth = windowWidth;
                } else {
                    $('#main_navigator').attr('style', 'display:block;overflow:auto;height:' + myHeight + 'px;');
                    contentWidth = windowWidth - $('#main_navigator').width() - 10;
                }


                viewer.Resize(contentWidth, contentHeight);
        ...
        OV.InitModelLoader(this.modelLoader, {
            onStart: () => {
                //todo
                $('.ov_svg_icon')[0].remove();

                this.viewer.SetLoadingModelViewer(true);

                this.ClearModel();
                this.SetUIState(OV.WebsiteUIState.Loading);
            },
            onFinish: (importResult, threeMeshes) => {
                AddButtonMe(this.toolbar, this.eventHandler, 'menu_black_24dp', 'Open Menu', null, this.viewer, () => {
                    this.OpenFileBrowserDialog();
                });
                ...
                this.viewer.SetLoadingModelViewer(false);

                // todo
                // setTimeout(()=> {
                //     $('#main_sidebar > div .ov_sidebar_content').eq(0).css('height', '200px');
                //     $('#main_sidebar > div .ov_sidebar_content').eq(1).css('height', '200px');
                //
                //     // $('#main_sidebar').insertAfter($('#main'));
                //     // $('#main_sidebar').insertBefore($('#main_sidebar'), $('#main').firstChild);
                // }, 200);
    ...
    InitSidebar() {
        ...
        function ToggleSidebar(sidebar, cookieHandler, sidebarPanels, panelId) {
            if ($('#main_sidebar').css('display') === 'none') {
        ...
        for (let id = 0; id < sidebarPanels.length; id++) {
            ...
            sidebarPanel.button = AddSidebarButton(this.toolbar, this.eventHandler, sidebarPanel, () => {
                //todo
                $('#main_navigator').attr('style', 'display:none');
        ...
        //todo
        let show = this.cookieHandler.GetBoolVal('ov_show_sidebar', false);
